...
def test_handler_error_does_not_leak_detail(monkeypatch):
    def explode():
        raise RuntimeError("connection to 10.1.2.3 refused")

    monkeypatch.setattr(app, "_get_table", explode)

    result = app.handler({}, {})

    assert result["statusCode"] == 500
    assert json.loads(result["body"]) == {"message": "counter error"}
