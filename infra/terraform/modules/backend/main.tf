...
locals {
  ...
  lambda_function_name = "${var.project_name}-visitor-counter"
}
...
resource "random_password" "counter_ip_salt" {
  ...
  # imported during the state recovery. the provider doesn't know how the
  # value was made so it thinks special = true and wants to replace it. the
  # stored result is the live salt, only the metadata is wrong. replacing it
  # would change every visitor fingerprint, so leave it.
  lifecycle {
...
resource "aws_lambda_function" "counter" {
  ...
  # terraform owns the function, deploy-backend.yml owns the code in it. the
  # archive_file above is just so a build from scratch works. without these
  # ignores an apply puts my local checkout back over whatever CI deployed.
  lifecycle {
